...
#define PROGRAM_FILE "hello_kernel.cl" // Define the name of the kernel file
#define KERNEL_NAME "hello_kernel"     // Define the default kernel name
...
int main(int argc, char **argv)
{
    ...
    /* Data and buffers */
    char getting_started_msg[16]; // Buffer to hold the message

// Number of kernels
#define N 6

    // Names of the kernels to execute
    char kernel_names[N][20] = {"hello_kernel_16", "hello_kernel_hi", "hello_kernel_lo", "hello_kernel_odd", "hello_kernel_even", "hello_kernel_s045e"};

    // Loop through each kernel
    for (int i = 0; i < N; ++i)
    {
        printf("Kernel: %s\t", kernel_names[i]);

        // Create the kernel
        cl_kernel kernel = clCreateKernel(program, kernel_names[i], &err);
        CHECK_CL_ERROR(err);

        // Create buffer for the kernel
        cl_mem getting_started_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 16, NULL, &err);
        CHECK_CL_ERROR(err);

        // Set the kernel argument
        CHECK_CL_ERROR(clSetKernelArg(kernel, 0, sizeof(cl_mem), &getting_started_buffer));
        ...
        CHECK_CL_ERROR(clEnqueueReadBuffer(queue, getting_started_buffer, CL_TRUE, 0, 16, getting_started_msg, 0, NULL, NULL));

        // Print the message
        printf("Message: \"%s\"\n", getting_started_msg);

        // Release the buffer and kernel
        CHECK_CL_ERROR(clReleaseMemObject(getting_started_buffer));
