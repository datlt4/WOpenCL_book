...
int main(int argc, char **argv)
{
    ...
    // Create memory buffer for 'a' and copy the initial value
    cl_mem a_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(a), &a, &err);
    CHECK_CL_ERROR(err);

    // Create memory buffer for 'b' and copy the initial value
    cl_mem b_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(b), &b, &err);
    CHECK_CL_ERROR(err);

    // Create memory buffer for 'result' with no initial value
    cl_mem out_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float), NULL, &err);
    CHECK_CL_ERROR(err);

    // Set the arguments of the kernel
    CHECK_CL_ERROR(clSetKernelArg(kernel, 0, sizeof(a_buffer), &a_buffer));
    CHECK_CL_ERROR(clSetKernelArg(kernel, 1, sizeof(b_buffer), &b_buffer));
    CHECK_CL_ERROR(clSetKernelArg(kernel, 2, sizeof(out_buffer), &out_buffer));

    // Execute the kernel
    CHECK_CL_ERROR(clEnqueueTask(queue, kernel, 0, NULL, NULL));

    // Read the output buffer back to the 'result' variable
    CHECK_CL_ERROR(clEnqueueReadBuffer(queue, out_buffer, CL_TRUE, 0, sizeof(float), &result, 0, NULL, NULL));
    printf("out: %f\n", result); // Print the result

    // Clean up resources
    CHECK_CL_ERROR(clReleaseMemObject(a_buffer));
    CHECK_CL_ERROR(clReleaseMemObject(b_buffer));
    CHECK_CL_ERROR(clReleaseMemObject(out_buffer));
