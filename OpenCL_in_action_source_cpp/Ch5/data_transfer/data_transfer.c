...
int main(int argc, char **argv)
{
    ...
    cl_kernel kernel = clCreateKernel(program, "transfer_global_to_local", &err); // Create kernel from the program
    CHECK_CL_ERROR(err);                                                          // Check for errors during kernel creation

    int in_data[4] = {14, 5, 24, 8};                                                             // Array to hold results read from the buffer
    int out_data[4];
    cl_mem check_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(in_data), NULL, &err); // Create buffer
